...
function Circuits() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ["products", "circuits", page, status],
    queryFn: async () => {
      const { data } = await axios.post<PaginatedResponse<Product>>(
        `/api/product/circuit/paginate`,
        {
          filters: {
            status,
          },
  ...
  return (
    <PageLayout
      title="Circuits"
      description="Manage your circuits."
      actions={
        <>
          <ProductFilter />
          <Link href="/admin/product/circuit/new">
            <Button size="sm" className="gap-1">
              <PlusCircle size={16} />
              <span className="sr-only sm:not-sr-only">Add Circuit</span>
            </Button>
          </Link>
        </>
      }
            ...
            data?.map((product) => (
              <TableRow className="*:py-2" key={product.id}>
                ...
                <TableCell className="w-20">
                  <ProductActions id={product.id} type="circuit" />
                </TableCell>
              </TableRow>
            ))}
        </TableBody>
